...
public class MyNUnit
{
    ...
    {
        this.path = path ?? throw new ArgumentNullException("path can't be empty");
    }
    ...
    public List<AssemblyReport> RunTests()
    {
        ...
        foreach (var dll in Directory.GetFiles(this.path, "*.dll"))
        {
            ...
            Parallel.ForEach(assembly.GetTypes(), type => {
            {
                ...
                if (classReport != null)
                {
                    lock (assemblyReport.ClassReports)
                    {
    ...
    where T : Attribute
    {
        foreach (var method in type.GetMethods().Where(m => m.GetCustomAttributes(typeof(T), false).Any()))
        {
            ...
            {
                LogError($"Error in method {method.Name}: {ex.Message}");
            }
        }
    }
    ...
    where T : Attribute
    {
        foreach (var method in type.GetMethods().Where(m => m.GetCustomAttributes(typeof(T), false).Any()))
        {
            ...
            {
                LogError($"Error in static method {method.Name}: {ex.Message}");
                return false;
            }
        }
    ...
    private static void LogError(string message)
    {
        try
        {
            ...
            string logFile = Path.Combine(logPath, $"error_{DateTime.Now:2025-02-23}.log");
            File.AppendAllText(logFile, $"{DateTime.Now:2025-02-23 12:00:00} - {message}{Environment.NewLine}");
        }
        catch (Exception e)
        {
    ...
    private TestReport ExecuteTest(MethodInfo method, Type type)
    {
        ...
        catch(Exception ex)
        {
            ...
            report.Reason = "Constructer wasn't create.";
        }
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    report.State = TestResult.FAILED;
                    report.Reason = "Expected exception wasn't thrown.";
                }
            }
